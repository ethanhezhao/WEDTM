...
void GNBP_mex_collapsed_deep(double *ZSDS, double *ZSWS, double *n_dot_k,
        ...
        double *shape, double *eta, double *eta_sum, double *prob_cumsum)
{
...
//                 Kvalid = Kvalid-1;*/
        ...
        for (cum_sum=0, k=0; k<Ksize; k++) {
            cum_sum += (eta[k+Ksize*v]+ ZSWS[k+Ksize*v])/(eta_sum[k] + n_dot_k[k])*(ZSDS[k+Ksize*j] + shape[k+Ksize*j]);
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    double *WS, *DS, *shape, *eta, *eta_sum, *prob_cumsum;
    ...
    GNBP_mex_collapsed_deep(ZSDS, ZSWS, n_dot_k, ZS, WS, DS, Vsize, Nsize, Ksize, WordNum, shape, eta, eta_sum, prob_cumsum);
